    ...
    exportAccount: async (accountId: string, exportedAt = new Date()) => {
      ...
      return {
        ...
        trades: tradeRows.map(({ trade, strategyName, playbookName, closeReasonName }, index) => ({
          ...
          mfe_price: trade.mfePrice == null ? null : Number(trade.mfePrice),
          mae_price: trade.maePrice == null ? null : Number(trade.maePrice),
      ...
      return db.transaction(async (tx) => {
        ...
        for (const trade of source.trades) {
          ...
          const [created] = await tx.insert(trades).values({
            ...
            realizedPnl: numeric(evaluation.preview.realizedPnl), realizedR: numeric(evaluation.preview.realizedR),
            mfePrice: numeric(trade.mfePrice), maePrice: numeric(trade.maePrice),
            mfeAmount: numeric(evaluation.preview.mfeAmount), maeAmount: numeric(evaluation.preview.maeAmount),
            mfeR: numeric(evaluation.preview.mfeR), maeR: numeric(evaluation.preview.maeR),
            capturedMovePct: numeric(evaluation.preview.capturedMovePct), confidence: trade.confidence,
