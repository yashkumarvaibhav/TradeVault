...
export function TradeEntryForm({ initialEntryAt, saved = false }: { initialEntryAt: string; saved?: boolean }) {
  const [state, formAction, pending] = useActionState<TradeFormState, FormData>(createTradeAction, {});
  ...
  return <form action={formAction} className="pb-24" noValidate>
      ...
      <div className="space-y-6">
        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-xl text-ink">Position</h2><p className="mb-5 text-sm text-muted">What you traded and how the position was structured.</p>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Field label="Instrument / symbol" name="symbol" error={errors.symbol}><Input id="symbol" name="symbol" value={v.symbol} onChange={(e) => update("symbol", e.target.value.toUpperCase())} placeholder="NIFTY, RELIANCE, EURUSD" autoFocus aria-invalid={!!errors.symbol} /></Field>
        ...
        </div>{v.status === "closed" ? <div className="mt-5 grid gap-4 border-t border-line pt-5 sm:grid-cols-2"><Field label="Exit date & time" name="exitAt" error={errors.exitAt}><Input id="exitAt" name="exitAt" type="datetime-local" value={v.exitAt} onChange={(e) => update("exitAt", e.target.value)} /></Field><Field label="Exit price" name="exitPrice" error={errors.exitPrice}><Input id="exitPrice" name="exitPrice" type="number" step="any" value={v.exitPrice} onChange={(e) => update("exitPrice", e.target.value)} /></Field>{v.assetClass === "Forex" ? <Field label="Manual P&L fallback" name="manualPnl" error={errors.manualPnl} hint="Signed amount in the selected currency; overrides price-based P&L."><Input id="manualPnl" name="manualPnl" type="number" step="any" value={v.manualPnl} onChange={(e) => update("manualPnl", e.target.value)} /></Field> : null}</div> : null}</section>

        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-xl text-ink">Context & discipline</h2><p className="mb-5 text-sm text-muted">Optional facts that make review useful later.</p><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Trading style" name="tradingStyle"><Input id="tradingStyle" name="tradingStyle" placeholder="Intraday, swing…" /></Field><Field label="Platform" name="platform"><Input id="platform" name="platform" placeholder="Zerodha, IBKR…" /></Field><Field label="Subcategory" name="subcategory"><Input id="subcategory" name="subcategory" placeholder="Large cap, major pair…" /></Field><Field label="Confidence (1–5)" name="confidence" error={errors.confidence}><select id="confidence" name="confidence" value={v.confidence} onChange={(e) => update("confidence", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{[1,2,3,4,5].map((n) => <option key={n} value={n}>{"★".repeat(n)}</option>)}</select></Field><Field label="Emotion" name="emotion"><select id="emotion" name="emotion" value={v.emotion} onChange={(e) => update("emotion", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{emotions.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Tags" name="tags" hint="Comma-separated"><Input id="tags" name="tags" placeholder="breakout, A+ setup" /></Field></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Rule violations" name="ruleViolations"><textarea id="ruleViolations" name="ruleViolations" rows={3} className="w-full rounded-md border border-line bg-raised p-3 text-sm text-ink" placeholder="Leave blank when rules were followed." /></Field><Field label="Linked note" name="linkedNote"><textarea id="linkedNote" name="linkedNote" rows={3} className="w-full rounded-md border border-line bg-raised p-3 text-sm text-ink" placeholder="Pre-trade thesis or journal link." /></Field></div><Field label="Trade notes" name="notes"><textarea id="notes" name="notes" rows={4} className="w-full rounded-md border border-line bg-raised p-3 text-sm text-ink" placeholder="What made this trade worth taking?" /></Field></section>
      </div>
    ...
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-page/95 p-3 backdrop-blur md:left-64"><div className="mx-auto flex max-w-[1540px] items-center justify-end gap-2"><Button asChild variant="ghost"><a href="/trades">Cancel</a></Button><Button type="submit" name="intent" value="another" variant="secondary" disabled={pending}><Plus aria-hidden="true" />Save & add another</Button><Button type="submit" name="intent" value="done" disabled={pending}>{pending ? "Saving…" : "Save trade"}</Button></div></div>
  </form>;
}
