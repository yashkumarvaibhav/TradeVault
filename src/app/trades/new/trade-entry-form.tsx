...
import {
  ...
  domesticAssetChoices,
  internationalAssetChoices,
  marketModeForTrade,
  tradeEntryProfile,
  underlyingChoices,
  type TradeAssetChoice,
  type TradeMarketMode,
} from "@/lib/trade-entry-profile";
...
export function TradeEntryForm({
  ...
  const evaluated = evaluateTradeEntry({
    ...
    mfePrice: numberOrNull(v.mfePrice), maePrice: numberOrNull(v.maePrice),
  ...
  const applyInstrumentDefaults = (symbol: string) => {
    ...
    setV((current) => ({
      ...
      expiryDate: savedInstrument.expiryDate ?? "",
      optionSide: savedInstrument.optionSide === "Call" || savedInstrument.optionSide === "Put" ? savedInstrument.optionSide : "",
      strikePrice: savedInstrument.strikePrice == null ? "" : String(Number(savedInstrument.strikePrice)),
  ...
  const chooseAsset = (choice: TradeAssetChoice) => {
    const next = classificationForChoice(choice, marketMode, v.assetClass);
    setAssetChoice(choice);
    setV((current) => ({
      ...current,
      ...next,
      multiplier: tradeEntryProfile(choice, marketMode).showMultiplier ? current.multiplier || "1" : "1",
      expiryDate: choice === "Futures" || choice === "Options" ? current.expiryDate : "",
      optionSide: choice === "Options" ? current.optionSide : "",
      strikePrice: choice === "Options" ? current.strikePrice : "",
      manualPnl: choice === "Forex" ? current.manualPnl : "",
      fxToAccount: choice === "Forex" ? current.fxToAccount || "1" : "1",
    }));
    setDefaultsApplied("");
  };
  ...
  return <form action={formAction} className="pb-24" noValidate>
    ...
    <input type="hidden" name="assetClass" value={v.assetClass} />
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-6">
        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-xl text-ink">{assetChoice} position</h2><p className="mb-5 text-sm text-muted">Relevant contract and sizing fields for this asset class.</p>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {profile.showUnderlying ? <Field label="Underlying market" name="underlyingAsset"><select id="underlyingAsset" value={v.assetClass} onChange={(e) => update("assetClass", e.target.value as AssetClass)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink">{underlyingChoices(marketMode).map((item) => <option key={item} value={item}>{item === "Equity" && marketMode === "international" ? "US Equity" : item}</option>)}</select></Field> : null}
            <Field label={profile.symbolLabel} name="symbol" error={errors.symbol} hint={libraries.instruments.length ? "Choose a saved symbol to reuse its defaults, or type a new one." : "Your first save creates a reusable instrument default."}><Input id="symbol" name="symbol" list="saved-instruments" value={v.symbol} onChange={(e) => applyInstrumentDefaults(e.target.value)} placeholder={profile.symbolPlaceholder} autoFocus autoComplete="off" aria-invalid={!!errors.symbol} /><datalist id="saved-instruments">{libraries.instruments.filter((item) => item.currency === v.currency).map((item) => <option key={item.id} value={item.symbol}>{assetChoiceForStored(item.assetClass, item.instrumentType)} · {item.currency}</option>)}</datalist>{defaultsApplied ? <p role="status" className="text-xs font-semibold text-accent">{defaultsApplied}</p> : null}</Field>
            <Field label="Trade currency" name="tradeCurrency" hint="Set by the Indian/international switch; P&L is never mixed."><Input id="tradeCurrency" value={v.currency} readOnly aria-readonly="true" className="font-semibold" /></Field>
            {profile.showExpiry ? <Field label="Contract expiry (optional)" name="expiryDate" error={errors.expiryDate}><Input id="expiryDate" name="expiryDate" type="date" value={v.expiryDate} onChange={(e) => update("expiryDate", e.target.value)} aria-invalid={!!errors.expiryDate} /></Field> : null}
            {profile.showOptionDetails ? <fieldset className="space-y-1.5"><legend className="text-sm font-medium text-ink">Option type</legend><div className="grid grid-cols-2 gap-2">{(["Call", "Put"] as const).map((side) => <label key={side} className={cn("flex min-h-11 cursor-pointer items-center justify-center rounded-md border text-sm font-semibold", v.optionSide === side ? "border-line-strong bg-accent-soft text-ink" : "border-line text-muted")}><input className="sr-only" type="radio" name="optionSide" value={side} checked={v.optionSide === side} onChange={() => update("optionSide", side)} />{side}</label>)}</div></fieldset> : null}
            {profile.showOptionDetails ? <Field label="Strike price (optional)" name="strikePrice" error={errors.strikePrice}><Input id="strikePrice" name="strikePrice" type="number" min="0" step="any" value={v.strikePrice} onChange={(e) => update("strikePrice", e.target.value)} aria-invalid={!!errors.strikePrice} /></Field> : null}
        ...
        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-xl text-ink">Entry, risk & target</h2><p className="mb-5 text-sm text-muted">Directional checks update as you type.</p><div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label={profile.entryPriceLabel} name="entryPrice" error={errors.entryPrice}><Input id="entryPrice" name="entryPrice" type="number" step="any" value={v.entryPrice} onChange={(e) => update("entryPrice", e.target.value)} aria-invalid={!!errors.entryPrice} /></Field>
          ...
          <Field label={profile.feesLabel} name="fees" error={errors.fees}><Input id="fees" name="fees" type="number" min="0" step="any" value={v.fees} onChange={(e) => update("fees", e.target.value)} /></Field>
        ...
        </div>{v.status === "closed" ? <div className="mt-5 grid gap-4 border-t border-line pt-5 sm:grid-cols-2"><Field label="Exit date & time" name="exitAt" error={errors.exitAt}><Input id="exitAt" name="exitAt" type="datetime-local" value={v.exitAt} onChange={(e) => update("exitAt", e.target.value)} /></Field><Field label={assetChoice === "Options" ? "Exit premium" : "Exit price"} name="exitPrice" error={errors.exitPrice}><Input id="exitPrice" name="exitPrice" type="number" step="any" value={v.exitPrice} onChange={(e) => update("exitPrice", e.target.value)} /></Field><Field label="Close reason" name="closeReasonId"><select id="closeReasonId" name="closeReasonId" value={v.closeReasonId} onChange={(e) => update("closeReasonId", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{libraries.closeReasons.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>{assetChoice === "Forex" ? <Field label="Manual P&L fallback" name="manualPnl" error={errors.manualPnl} hint="Signed amount in USD; overrides price-based P&L."><Input id="manualPnl" name="manualPnl" type="number" step="any" value={v.manualPnl} onChange={(e) => update("manualPnl", e.target.value)} /></Field> : null}</div> : null}</section>

        {v.status === "closed" ? <section className="rounded-lg border border-line bg-raised p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-serif text-xl text-ink">Manual excursion capture</h2><p className="mt-1 text-sm text-muted">Optional observed extrema while this position was open. TradeVault does not infer candles or broker data.</p></div><span className="rounded-sm border border-line bg-sidebar px-2.5 py-1 text-xs font-semibold text-muted">Manual evidence</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Maximum favorable price" name="mfePrice" error={errors.mfePrice ?? evaluated.errors.mfePrice} hint={v.direction === "Long" ? "Highest observed price from entry through exit." : "Lowest observed price from entry through exit."}><Input id="mfePrice" name="mfePrice" type="number" min="0" step="any" value={v.mfePrice} onChange={(e) => update("mfePrice", e.target.value)} aria-invalid={!!(errors.mfePrice ?? evaluated.errors.mfePrice)} /></Field><Field label="Maximum adverse price" name="maePrice" error={errors.maePrice ?? evaluated.errors.maePrice} hint={v.direction === "Long" ? "Lowest observed price from entry through exit." : "Highest observed price from entry through exit."}><Input id="maePrice" name="maePrice" type="number" min="0" step="any" value={v.maePrice} onChange={(e) => update("maePrice", e.target.value)} aria-invalid={!!(errors.maePrice ?? evaluated.errors.maePrice)} /></Field></div><dl className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Metric label="MFE" value={evaluated.preview.mfeR == null ? "—" : `${evaluated.preview.mfeR.toFixed(2)}R`} sub={evaluated.preview.mfeAmount == null ? "No favorable price" : money(evaluated.preview.mfeAmount)} /><Metric label="MAE" value={evaluated.preview.maeR == null ? "—" : `${evaluated.preview.maeR.toFixed(2)}R`} sub={evaluated.preview.maeAmount == null ? "No adverse price" : money(evaluated.preview.maeAmount)} /><Metric label="Captured move" value={evaluated.preview.capturedMovePct == null ? "—" : `${evaluated.preview.capturedMovePct.toFixed(1)}%`} sub="Realized P&L ÷ MFE" /><Metric label="Evidence" value={`${Number(v.mfePrice !== "") + Number(v.maePrice !== "")}/2`} sub="Manual extrema supplied" /></dl></section> : null}

        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-xl text-ink">Context & discipline</h2><p className="mb-5 text-sm text-muted">Link the trade to repeatable setups and preserve the facts that make review useful.</p><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Strategy" name="strategyId"><select id="strategyId" name="strategyId" value={v.strategyId} onChange={(e) => update("strategyId", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{libraries.strategies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="Playbook" name="playbookId"><select id="playbookId" name="playbookId" value={v.playbookId} onChange={(e) => update("playbookId", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{libraries.playbooks.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="Trading style" name="tradingStyle"><Input id="tradingStyle" name="tradingStyle" value={v.tradingStyle} onChange={(e) => update("tradingStyle", e.target.value)} placeholder="Intraday, swing…" /></Field><Field label="Platform" name="platform"><Input id="platform" name="platform" value={v.platform} onChange={(e) => update("platform", e.target.value)} placeholder="Zerodha, IBKR…" /></Field><Field label={profile.subcategoryLabel} name="subcategory"><Input id="subcategory" name="subcategory" value={v.subcategory} onChange={(e) => update("subcategory", e.target.value)} placeholder={profile.subcategoryPlaceholder} /></Field><Field label="Confidence (1–5)" name="confidence" error={errors.confidence}><select id="confidence" name="confidence" value={v.confidence} onChange={(e) => update("confidence", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{[1,2,3,4,5].map((n) => <option key={n} value={n}>{"★".repeat(n)}</option>)}</select></Field><Field label="Emotion" name="emotion"><select id="emotion" name="emotion" value={v.emotion} onChange={(e) => update("emotion", e.target.value)} className="h-11 w-full rounded-md border border-line bg-raised px-3 text-sm text-ink"><option value="">Not set</option>{emotions.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Tags" name="tags" hint="Comma-separated"><Input id="tags" name="tags" defaultValue={initialText?.tags ?? ""} placeholder="breakout, A+ setup" /></Field></div>
