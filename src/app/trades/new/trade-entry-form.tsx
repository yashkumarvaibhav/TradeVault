...
export function TradeEntryForm({ initialEntryAt, libraries, saved = false }: { initialEntryAt: string; libraries: TradeEntryLibraries; saved?: boolean }) {
  ...
  const [templateId, setTemplateId] = React.useState(libraries.checklistTemplates[0]?.id ?? "");
  const [setupChecklist, setSetupChecklist] = React.useState<SetupChecklistItem[]>(() =>
    (libraries.checklistTemplates[0]?.items ?? []).map((item) => ({ ...item, completed: false })),
  );
  ...
  return <form action={formAction} className="pb-24" noValidate>
    ...
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-page/95 p-3 backdrop-blur md:left-64"><div className="mx-auto flex max-w-[1540px] items-center justify-end gap-2"><Button asChild variant="ghost"><Link href="/trades">Cancel</Link></Button><Button type="submit" name="intent" value="another" variant="secondary" disabled={pending}><Plus aria-hidden="true" />Save & add another</Button><Button type="submit" name="intent" value="done" disabled={pending}>{pending ? "Saving…" : "Save trade"}</Button></div></div>
  </form>;
}
