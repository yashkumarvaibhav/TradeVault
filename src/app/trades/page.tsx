...
export default async function TradesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const repository = createTradeRepository(getDb(), scope);
  const [result, options] = await Promise.all([repository.queryPage(query), repository.filterOptions(account.id)]);
  ...
  const filterKeys = ["q", "asset", "status", "result", "direction", "currency", "instrument", "symbol", "subcategory", "style", "platform", "emotion", "from", "to"];
