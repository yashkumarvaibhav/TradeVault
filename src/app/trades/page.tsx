...
import { assetClasses, instrumentTypes, tradeDirections, tradeStatuses } from "@/db/schema";
...
export default async function TradesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  Object.entries(raw).forEach(([key, value]) => { const item = first(value); if (item && !transientParams.has(key) && key !== "currency") url.set(key, item); });
  const query: TradeQueryOptions = {
    ...
    direction: allowed(first(raw.direction), tradeDirections), currency: shellUser.currency,
  ...
  const total = realized.reduce((sum, row) => sum + Number(row.realizedPnl), 0);
  const profitFactor = () => {
    const values = realized.map((row) => Number(row.realizedPnl));
  ...
  const filterKeys = ["q", "asset", "status", "result", "direction", "instrument", "symbol", "subcategory", "style", "platform", "emotion", "strategy", "playbook", "closeReason", "from", "to"];
  ...
  return <AppShell user={shellUser}>
    <PageHeader eyebrow={<><Chip tone="accent">{account.name} · {shellUser.currency}</Chip><Chip>{result.total} matching records</Chip><Chip>{timeZone}</Chip></>} title="My trades" description={`Search, filter, sort, and page through ${shellUser.currency}-only trades in the active market workspace.`} actions={<Button asChild><Link href="/trades/new"><Plus aria-hidden="true" />Add trade</Link></Button>} />
    ...
    <section aria-label="Filtered trade summary" className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-lg border border-line bg-raised p-4"><p className="text-xs font-semibold uppercase tracking-[0.08em] text-muted">Net P&amp;L · {shellUser.currency}</p><p className={"mt-2 font-serif text-2xl tnum " + (total >= 0 ? "text-profit" : "text-loss")}>{realized.length ? money(shellUser.currency, total) : "—"}</p><p className="mt-1 text-xs text-faint">Profit factor {profitFactor()} · filtered closed trades</p></div>
      ...
      <div className="rounded-lg border border-line bg-raised p-4"><p className="text-xs font-semibold uppercase tracking-[0.08em] text-muted">Average realized R</p><p className="mt-2 font-serif text-2xl text-ink tnum">{avgRValues.length ? `${(avgRValues.reduce((sum, value) => sum + value, 0) / avgRValues.length).toFixed(2)}R` : "—"}</p><p className="mt-1 text-xs text-faint">{shellUser.currency} workspace only</p></div>
    </section>
      ...
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-end justify-end gap-2">
          <details className="group relative">
            ...
            <form action="/trades" method="get" className="absolute left-0 z-30 mt-2 w-64 rounded-md border border-line bg-raised p-4 shadow-sm sm:left-auto sm:right-0">
