...
export default async function TradesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <AppShell user={shellUser}>
    <PageHeader eyebrow={<><Chip tone="accent">{account.name} · {account.defaultCurrency}</Chip><Chip>{result.total} matching records</Chip><Chip>{timeZone}</Chip></>} title="My trades" description="Search, filter, sort, and page through the complete tenant-scoped journal." actions={<Button asChild><Link href="/trades/new"><Plus aria-hidden="true" />Add trade</Link></Button>} />
