...
export default async function TradesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const result = await createTradeRepository(getDb(), scope).queryPage(query);
  ...
  const filterKeys = ["q", "asset", "status", "result", "direction", "currency", "instrument", "emotion", "from", "to"];
