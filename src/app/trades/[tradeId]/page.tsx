  ...
  return <AppShell user={shellUser}>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-6">
        <section className="rounded-lg border border-line bg-raised p-5"><h2 className="font-serif text-2xl text-ink">Execution</h2><p className="mt-1 text-sm text-muted">The historical position facts captured at entry and exit.</p><dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Fact label={trade.instrumentType === "Options" ? "Entry premium" : "Entry"} value={money(trade.currency, Number(trade.entryPrice))} detail={formatDateTimeInTimeZone(trade.entryAt, timeZone)} /><Fact label={trade.instrumentType === "Options" ? "Exit premium" : "Exit"} value={trade.exitPrice == null ? "—" : money(trade.currency, Number(trade.exitPrice))} detail={trade.exitAt ? formatDateTimeInTimeZone(trade.exitAt, timeZone) : "Position remains open"} /><Fact label={trade.instrumentType === "Cash" ? "Quantity" : "Lots / contracts"} value={`${Number(trade.quantity).toLocaleString()} × ${Number(trade.multiplier).toLocaleString()}`} detail="Quantity × multiplier" /><Fact label="Fees" value={money(trade.currency, Number(trade.fees))} detail={duration} />{trade.instrumentType !== "Cash" ? <Fact label="Underlying" value={trade.assetClass} detail={trade.expiryDate ? `Expires ${trade.expiryDate}` : "Expiry not recorded"} /> : null}{trade.instrumentType === "Options" ? <Fact label="Option contract" value={trade.optionSide ?? "Type not recorded"} detail={trade.strikePrice == null ? "Strike not recorded" : `Strike ${Number(trade.strikePrice).toLocaleString()}`} /> : null}</dl></section>
