...
export type CalendarMode = "recent" | "month" | "year";
...
function rangeFor(mode: CalendarMode, now: Date, month: string, year: number) {
  if (mode === "recent") {
...
function DayActivity({ date, day, currency }: { date: string; day?: CalendarDay; currency: Currency }) {
  const money = moneyFormatter(currency);
  return (
          ...
          <div className="mt-4 space-y-3">
            {day.trades.map((trade) => (
              <Link key={trade.id} href={`/trades/${trade.id}`} aria-label={`View ${trade.symbol} trade detail`} className="block rounded-md border border-line p-3 transition-colors hover:border-line-strong hover:bg-hover">
                ...
                <div className="mt-2 flex items-center justify-between gap-2 text-xs text-muted"><span>Entered {new Date(trade.entryAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", timeZone: "UTC" })}</span><span className="tnum">{trade.realizedR == null ? "R —" : `${trade.realizedR >= 0 ? "+" : ""}${trade.realizedR.toFixed(2)}R`}</span></div>
              </Link>
            ))}
...
function CalendarScope({ asset, currency, onAsset, onCurrency }: { asset: ScopeAsset; currency: Currency; onAsset: (asset: ScopeAsset) => void; onCurrency: (currency: Currency) => void }) {
  return <ScopeToolbar label="Calendar scope" note={<>Money cells are isolated to <strong className="text-ink">{currency}</strong>. INR and USD are never combined; switching asset or currency recomputes every day.</>}>
...
export function CalendarDashboard({ dataByAsset, nowIso, initialMode = "month", initialMonth, initialYear, initialDay }: { dataByAsset: CalendarDataByAsset; nowIso: string; initialMode?: CalendarMode; initialMonth: string; initialYear: number; initialDay?: string }) {
  const now = React.useMemo(() => new Date(nowIso), [nowIso]);
  const [asset, setAsset] = React.useState<ScopeAsset>("Overall");
  ...
  const initialRange = rangeFor(initialMode, now, initialMonth, initialYear);
  ...
  return <div className="space-y-6 lg:space-y-8">
    ...
    <CalendarScope asset={asset} currency={currency} onAsset={changeAsset} onCurrency={changeCurrency} />
    <Card>
      <CardHeader className="flex-col sm:flex-row sm:items-center"><div><CardTitle>Calendar mode</CardTitle><CardDescription>{mode === "recent" ? `${longDate(dateKey(visibleRange.start))}–${longDate(dateKey(visibleRange.end))}` : mode === "month" ? monthLabel(month) : String(year)} · exit-date outcomes and actual review dates</CardDescription></div><SegmentedControl type="single" value={mode} onValueChange={(value) => value && setMode(value as CalendarMode)} aria-label="Calendar mode"><SegmentedControlItem value="recent">Recent</SegmentedControlItem><SegmentedControlItem value="month">Month</SegmentedControlItem><SegmentedControlItem value="year">Year</SegmentedControlItem></SegmentedControl></CardHeader>
    </Card>
    <section className="grid min-w-0 gap-4 xl:grid-cols-12" aria-label={`${mode} calendar and selected day activity`}>
      <Card className="min-w-0 xl:col-span-8 2xl:col-span-9">
        <CardHeader className="flex-row items-center"><div><CardTitle>{mode === "recent" ? "Recent 42 days" : mode === "month" ? monthLabel(month) : `${year} intensity`}</CardTitle><CardDescription>{currency} · {asset} · selected cell has a teal outline</CardDescription></div>{mode !== "recent" ? <div className="flex items-center gap-1"><Button variant="ghost" size="icon" aria-label={mode === "month" ? "Previous month" : "Previous year"} onClick={() => mode === "month" ? moveMonth(-1) : moveYear(-1)}><ChevronLeft aria-hidden="true" /></Button><Button variant="ghost" size="icon" aria-label={mode === "month" ? "Next month" : "Next year"} onClick={() => mode === "month" ? moveMonth(1) : moveYear(1)}><ChevronRight aria-hidden="true" /></Button></div> : <Clock3 className="size-5 text-muted" aria-hidden="true" />}</CardHeader>
        <CardContent>{mode === "recent" ? <RecentGrid data={data} currency={currency} now={now} selected={selected} onSelect={setSelected} /> : mode === "month" ? <MonthGrid data={data} currency={currency} month={month} selected={selected} onSelect={setSelected} /> : <YearGrid data={data} currency={currency} year={year} selected={selected} onSelect={setSelected} />}</CardContent>
      </Card>
      <div className="min-w-0 xl:col-span-4 2xl:col-span-3"><DayActivity date={selected} day={selectedDay} currency={currency} /></div>
    </section>
  </div>;
}
